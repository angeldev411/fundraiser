...
//import VolunteerDashboard from '../scripts/volunteer/dashboard.jsx';
...
const requireVolunteer = (req, res, next) => {
  ...
  let authorized = roles.some(function(role){
    return req.roles[role];
  });
...
const requireTeamLeader = (req, res, next) => {
  ...
  if(authorized){
    ...
    res.status(401).send("User not authorized by role, needs " + JSON.stringify(roles) + " has " + JSON.stringify(req.roles));
  }
};
const requireSuperAdmin = (req, res, next) => {
  const roles = [`project_${req.params.uuid}_creator`,
